	...
	def input(self):
		...
		if not self.attacking:
			...
			if keys[pygame.K_SPACE]:
				...
				tmp = self.status.split('_')[0]
				if tmp == 'left': self.bullet_direction = vector(-1,0)
				elif tmp == 'right': self.bullet_direction = vector(1,0)
				elif tmp == 'up': self.bullet_direction = vector(0,-1)
				elif tmp == 'down': self.bullet_direction = vector(0,1)
